...
const users = {
  ...
  findById: async (id) => {
    ...
    if (!pErr && profile) {
      ...
      return { id: profile.id, username: profile.username, email: email, displayName: profile.display_name, profilePic: profile.profile_pic, is2FAEnabled: profile.is2fa_enabled || 'none', isAdmin: profile.is_admin, magicLinkEmail: profile.magic_link_email, email2FAEmail: profile.email2fa_email };
    }
    if (pErr) throw pErr;
    return null;
  },
  ...
  update: async (id, updates) => {
    ...
    if ('email2FAEmail' in updates) payload.email2fa_email = updates.email2FAEmail;
